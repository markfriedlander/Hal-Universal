...
/// from the text view it points at. Numbers are placed from the layoutManager's line-fragment rects,
/// so a number appears ONLY at the first visual row of a real (newline-delimited) line — a soft-wrapped
/// continuation row gets no number. That absence is the "wrap clarity": a line that spills onto two
/// rows visibly owns both rows under one number, so you can always tell a wrap from a new line.
final class LineNumberGutter: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let attrs: [NSAttributedString.Key: Any] = [
            .font: numberFont, .foregroundColor: UIColor.secondaryLabel,
        ]
        let s = (tv.text ?? "") as NSString

        // Draw one right-aligned number for the visual row whose top (in container space) is `cy`.
        func drawNumber(_ n: Int, atContainerY cy: CGFloat, rowHeight: CGFloat) {
            let str = String(n) as NSString
            let size = str.size(withAttributes: attrs)
            let y = cy + insetTop - offsetY + (rowHeight - size.height) / 2
            let x = gutterWidth - 6 - size.width
            str.draw(at: CGPoint(x: x, y: y), withAttributes: attrs)
        }

        if s.length == 0 {
            drawNumber(1, atContainerY: 0, rowHeight: numberFont.lineHeight)
        } else {
            var lineNo = 1
            s.enumerateSubstrings(in: NSRange(location: 0, length: s.length),
                ...
                drawNumber(lineNo, atContainerY: frag.minY, rowHeight: frag.height)
                lineNo += 1
            }
            // A trailing newline means an empty final line the enumeration doesn't emit; the
            // layoutManager tracks it as the "extra" fragment. Number it so a script ending in a
            // blank line still shows that line's number (matching how the cursor sits there).
            if s.hasSuffix("\n") {
                let extra = lm.extraLineFragmentRect
                drawNumber(lineNo, atContainerY: extra.minY,
                           rowHeight: extra.height > 0 ? extra.height : numberFont.lineHeight)
            }
        }
...
final class GutterTextView: UITextView {
    ...
    /// Force a gutter refresh after a text change that may not itself trigger layout (e.g. an edit
    /// that doesn't change content size). Cheap: schedules one layout pass.
...
/// keyboard can't turn "--" into an em dash or "" into curly quotes inside a script. Stage 2a adds the
/// line-number gutter via the `GutterTextView` subclass (2b will tint error lines red).
