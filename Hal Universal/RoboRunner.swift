...
/// tell a wrap from a new line. Stage 2b: a line the coach flags is drawn in red (error) or orange
/// (warning) instead of the usual secondary gray, matching the `LineHighlightView` band behind it.
final class LineNumberGutter: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let errors = tv.errorLines
        let warnings = tv.warningLines

        // One right-aligned number per real line, colored by the coach's verdict for that line.
        tv.forEachLineRect { n, firstFrag, _ in
            let color: UIColor = errors.contains(n) ? .systemRed
                : (warnings.contains(n) ? .systemOrange : .secondaryLabel)
...
/// A faint full-width band drawn BEHIND the text on any line the coach flags (red for an error, orange
/// for a warning). It is a passive subview inserted below the text container (with a negative
/// `zPosition` so it always renders behind the glyphs) and pinned to the visible area like the gutter.
/// Unlike the gutter's per-line number, the band uses the line's FULL bounding rect, so a flagged line
/// that soft-wraps onto several rows is tinted across all of them. Draws nothing when the coach is clean.
final class LineHighlightView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        tv.forEachLineRect { n, firstFrag, fullBounds in
            let fill: UIColor
            if errors.contains(n) { fill = UIColor.systemRed.withAlphaComponent(0.14) }
            else if warnings.contains(n) { fill = UIColor.systemOrange.withAlphaComponent(0.12) }
            else { return }
...
final class GutterTextView: UITextView {
    ...
    /// 1-based line numbers the coach flags, set by `RoboScriptEditor` from the live validator output.
    /// The gutter colors these numbers and the highlight bands them; both read from here.
    private(set) var errorLines: Set<Int> = []
    private(set) var warningLines: Set<Int> = []
    ...
    /// Update the flagged-line sets and redraw the gutter + band only if they actually changed.
    /// Positions don't move, so this is a repaint (setNeedsDisplay), not a relayout.
    func setIssueLines(errors: Set<Int>, warnings: Set<Int>) {
        guard errors != errorLines || warnings != warningLines else { return }
        errorLines = errors
...
/// error lines show red (warnings orange) in the gutter with a faint band behind them.
struct RoboScriptEditor: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> GutterTextView {
        ...
        tv.text = text
        tv.setIssueLines(errors: errorLines, warnings: warningLines)
        return tv
    }
...
struct RoboEditorView: View {
    ...
    // The coach's flagged lines, fed to the editor's gutter (red = error, orange = warning). Only
    // line-specific issues highlight; whole-script issues (line 0) still surface in the Check sheet.
    // Error wins if a line has both, so warnings never mask an error line's red.
    private var errorLines: Set<Int> { Set(liveIssues.filter { $0.isError && $0.line > 0 }.map { $0.line }) }
    private var warningLines: Set<Int> {
        Set(liveIssues.filter { !$0.isError && $0.line > 0 }.map { $0.line }).subtracting(errorLines)
    }
    ...
    private func draftFromField() {
        ...
        if liveErrorCount == 0 && stepCount >= 2 {
