...
class DocumentImportManager: ObservableObject {
    ...
    func importDocuments(from urls: [URL], chatViewModel: ChatViewModel) async {
        ...
        for url in urls {
            ...
            if !hasAccess {
                ...
                skippedFiles.append(SkippedFile(name: url.lastPathComponent, reason: "Hal couldn't get permission to open \(url.lastPathComponent)."))
                continue
            }
    ...
    func processURLImmediatelyWithEntities(_ url: URL) async -> ([ProcessedDocument], [SkippedFile]) {
        var processedFiles: [ProcessedDocument] = []
        var skippedFiles: [SkippedFile] = []
        ...
        guard FileManager.default.fileExists(atPath: url.path, isDirectory: &isDirectory) else {
            ...
            skippedFiles.append(SkippedFile(name: url.lastPathComponent, reason: "\(url.lastPathComponent) couldn't be found."))
        ...
        if isDirectory.boolValue {
            do {
                ...
                skippedFiles.append(SkippedFile(name: url.lastPathComponent, reason: "Couldn't read the contents of the folder \(url.lastPathComponent)."))
            }
        } else {
            do {
                let processed = try await processDocumentImmediatelyWithEntities(url)
                ...
                // Keep the extractor's specific, user-facing reason (it names the file).
                let reason = (error as? DocumentProcessingError)?.errorDescription
                    ?? "\(url.lastPathComponent) couldn't be read (\(error.localizedDescription))."
                skippedFiles.append(SkippedFile(name: url.lastPathComponent, reason: reason))
                print("HALDEBUG-IMPORT: Skipped \(url.lastPathComponent): \(reason)")
            }
        }
        return (processedFiles, skippedFiles)
    }
    ...
    // THROWS a DocumentProcessingError on any failure (unsupported type, too large, no
    // readable text, extraction error) so the caller can surface the REASON to the user,
    // instead of the old silent `return nil` that collapsed every failure into an
    // anonymous skipped-filename with no explanation.
    private func processDocumentImmediatelyWithEntities(_ url: URL) async throws -> ProcessedDocument {
        print("HALDEBUG-IMPORT: Processing document with entity extraction: \(url.lastPathComponent)")

        let fileExtension = url.pathExtension.lowercased()

        // Format check. .doc is allowed through to extractContent's switch, which throws
        // the actionable "save as .docx" message rather than a generic unsupported error.
        guard isFormatSupported(fileExtension) || fileExtension == "doc" else {
        ...
        // Tiered file size checking (15 MB warning, 25 MB hard limit). Best-effort: if the
        // size can't be read, skip the check and let extraction proceed.
        if let fileSize = (try? FileManager.default.attributesOfItem(atPath: url.path))?[.size] as? Int64 {
            ...
            if fileSizeMB > 25.0 {
                ...
                throw DocumentProcessingError.fileTooLarge(url.lastPathComponent, fileSizeMB)
            }
            if fileSizeMB > 15.0 {
                await MainActor.run {
                    self.importProgress = "Processing large file: \(url.lastPathComponent) (\(String(format: "%.1f", fileSizeMB)) MB). This may take a minute or two..."
                }
                print("HALDEBUG-IMPORT: Large file warning: \(url.lastPathComponent) - \(String(format: "%.1f", fileSizeMB)) MB")
            }
        } else {
            print("HALDEBUG-IMPORT: Could not determine file size for \(url.lastPathComponent) - continuing")
        }

        // extractContent throws a specific DocumentProcessingError on failure; propagate it.
        let content = try extractContent(from: url, fileExtension: fileExtension)
        guard !content.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            print("HALDEBUG-IMPORT: No readable text in: \(url.lastPathComponent)")
    ...
    enum DocumentProcessingError: Error, LocalizedError {
        ...
        case noReadableText(String)       // extraction ran but found no text (e.g. a scanned/image PDF)
    ...
    /// A file the import couldn't take, paired with the human-readable reason why.
    /// The reason is the extractor's own `DocumentProcessingError` message (which names
    /// the file), so it can be shown to the user verbatim. Replaces the old bare-filename
    /// `[String]` skip list that discarded the reason before it reached anyone.
    struct SkippedFile {
        let name: String
        let reason: String
    }
    ...
    func generateImportMessages(documentSummaries: [String],
        ...
        let baseResponse = documentSummaries.count == 1
            ? "Understood! I've processed the document you shared. I'm ready for your questions."
            : "Got it! I've processed those \(documentSummaries.count) documents. What would you like to discuss about them?"
        // Append the skipped-file note so a PARTIAL import (some in, some failed) tells the
        // truth about both halves in one turn.
        let halResponse = baseResponse + skippedNote

        appendImportTurn(user: userMessageContent, hal: halResponse, chatViewModel: chatViewModel)
        print("HALDEBUG-IMPORT: Generated import conversation messages with entity context")
    }

    /// Appends a user+Hal message pair for a document import to the chat and persists the
    /// turn. Shared by the success, partial-skip, and all-skipped paths so they store
    /// identically. Synchronous; the caller is already @MainActor.
    private func appendImportTurn(user: String, hal: String, chatViewModel: ChatViewModel) {
        let turn = chatViewModel.memoryStore.getCurrentTurnNumber(conversationId: chatViewModel.conversationId) + 1
        chatViewModel.messages.append(ChatMessage(content: user, isFromUser: true, recordedByModel: "user", turnNumber: turn))
        chatViewModel.messages.append(ChatMessage(content: hal, isFromUser: false, recordedByModel: chatViewModel.selectedModel.id, turnNumber: turn))
        chatViewModel.memoryStore.storeTurn(
            conversationId: chatViewModel.conversationId,
            userMessage: user,
            assistantMessage: hal,
