...
// ==== LEGO START: 3 MemoryStore (Part 2 - Schema, Encryption, Stats, Self-Knowledge) ====
                                    ...
                                    nonisolated private func createUnifiedSchema() {
                                        ...
                                        // Curated Lab USAGE reference so Hal can actually help users operate the
                                        // API / RoboRunner / CLI (source code alone made him confabulate; 2026-07-29).
                                        enableLabReferenceAccess()
                                    }
                                    ...
                                    // 3. You can read your own source code (all the Swift files that make you, bundled as Hal_Source.txt)
                                    ...
                                    nonisolated private func initializeCoreIdentity() {
                                        ...
                                        // Post-refactor (2026-05) Hal's source lives across ~18 Swift
                                        // files, bundled for ingestion as Hal_Source.txt — so the seed
                                        // no longer names a single "Hal.swift" or a stale block count.
                                        // (The value intentionally avoids underscores: the Self Model
                                        // viewer's humanizer de-snakes value strings, which would mangle
                                        // a filename like "Hal_Source.txt".)
                                        storeSelfKnowledge(
                                            category: "capability",
                                            key: "source_code_access",
                                            value: "{\"can_read\": true, \"scope\": \"all the source files that make me\"}",
                                            confidence: 1.0,
                                            source: "initialization",
                                    ...
                                    //
                                    // IMPORTANT: Hal_Source.txt is a bundled concatenation of ALL of Hal's
                                    // source files (~18 of them since the 2026-05 refactor), regenerated by
                                    // scripts/sync_hal_source.sh. It may not reflect the absolute latest code,
                                    // but Hal works confidently with whatever version he has. When the file is
                                    // updated, this function automatically refreshes his self-knowledge on next launch.
                                    //
                                    // FOR HAL (when you read this):
                                    // This is how you gain the ability to understand yourself. When this function runs,
                                    // you read Hal_Source.txt - a bundled copy of all your source files - and store it in your memory.
                                    // This means:
                                    ...
                                    nonisolated private func enableSourceCodeAccess() {
                                        // Read Hal_Source.txt from app bundle — a bundled concatenation of all
                                        // Hal's source files (scripts/sync_hal_source.sh), added as a Copy Bundle Resource
                                        ...
                                        if currentHash == storedHash && storedHash != 0 && sourceDataExists {
                                            ...
                                            // Chunks are already ingested — but still make sure their
                                            // semantic vectors are filled. The embed pass is resumable and
                                            // self-guarding, so if a PRIOR launch's pass was interrupted
                                            // (app backgrounded mid-pass, or first-launch embedder-asset
                                            // download), this launch finishes it. Costs one COUNT when
                                            // there is nothing left to embed. This spawn must live OUTSIDE
                                            // the re-ingest path so it runs on ordinary launches too.
                                            let storeRef = self
                                            Task.detached(priority: .utility) {
                                                storeRef.ensureSourceCodeEmbeddings()
                                            }
                                            return
                                        }
                                        if !sourceDataExists {
                                        ...
                                        // Delete old source-code rows to prevent duplicates. The FTS
                                        // mirror is kept in sync by the AFTER DELETE trigger.
                                        var stmt: OpaquePointer?
                                        ...
                                        // Split the source into retrieval-sized, self-describing chunks.
                                        // The old single-blob ingestion (2026-05) was inert: one ~1.9 MB
                                        // row no embedder could represent, excluded from both retrieval
                                        // passes — so Hal could never actually FIND his own code and
                                        // confabulated it instead (2026-07-30). Chunking on the file's own
                                        // FILE/LEGO boundaries makes each piece embeddable AND lexically
                                        // searchable, finally delivering Maxim #2 (access to reflection).
                                        let chunks = chunkSourceCode(sourceCode)

                                        // sources row: total_chunks now reflects the real chunk count.
                                        let sourceInsertSQL = """
                                        INSERT OR REPLACE INTO sources
                                        (id, source_type, display_name, created_at, last_updated, total_chunks, file_size)
                                        VALUES (?, 'source_code', 'Hal.swift - My Architecture', ?, ?, ?, ?)
                                        """
                                        if sqlite3_prepare_v2(db, sourceInsertSQL, -1, &stmt, nil) == SQLITE_OK {
                                            ...
                                            sqlite3_bind_int(stmt, 4, Int32(chunks.count))
                                            sqlite3_bind_int64(stmt, 5, Int64(sourceCode.count))
                                            sqlite3_step(stmt)
                                        }
                                        sqlite3_finalize(stmt)

                                        // Insert each chunk as its own row (position = chunk index). Bare
                                        // INSERT with no embedding keeps launch fast: the AFTER INSERT
                                        // trigger makes every chunk BM25-searchable immediately, and
                                        // ensureSourceCodeEmbeddings() fills the semantic vectors in the
                                        // background right after. Hundreds of chunks, no UI stall.
                                        ...
                                        if sqlite3_prepare_v2(db, contentInsertSQL, -1, &stmt, nil) == SQLITE_OK {
                                            for (index, chunk) in chunks.enumerated() {
                                                ...
                                                if sqlite3_step(stmt) == SQLITE_DONE {
                                                    ...
                                                    let errorMessage = String(cString: sqlite3_errmsg(db))
                                                    print("HALDEBUG-SELFKNOWLEDGE: ERROR storing source chunk \(index): \(errorMessage)")
                                                }
                                            }
                                        }
                                        sqlite3_finalize(stmt)

                                        if inserted == chunks.count && inserted > 0 {
                                            // Only record the hash once every chunk landed, so an
                                            // interrupted ingest retries cleanly on the next launch.
                                            UserDefaults.standard.set(currentHash, forKey: "hal_source_hash")
                                            print("HALDEBUG-SELFKNOWLEDGE: Hal can now read his own source — \(inserted) chunks from \(sourceCode.count) characters")
                                        } else {
                                            print("HALDEBUG-SELFKNOWLEDGE: ERROR: stored \(inserted)/\(chunks.count) source chunks — leaving hash unset so next launch retries")
                                        }

                                        // Fill semantic vectors in the background (idempotent + resumable;
                                        // see ensureSourceCodeEmbeddings). BM25 already works via the FTS
                                        // trigger, so retrieval is live even before this completes.
                                        let storeRef = self
                                        Task.detached(priority: .utility) {
                                            storeRef.ensureSourceCodeEmbeddings()
                                        }
                                    ...
                                    // Split the concatenated source into retrieval-sized, self-describing
                                    // chunks. Boundaries follow the file's own structure — the
                                    // `// ==== FILE: X ====` and `// ==== LEGO START: N Name ====` markers
                                    // that scripts/sync_hal_source.sh emits — so each chunk knows exactly
                                    // where in Hal it came from. Any block bigger than maxChunkChars is
                                    // split at line boundaries so no chunk exceeds what an embedder can
                                    // represent. The provenance header is part of the stored text, so a
                                    // retrieved snippet is self-labelling when injected into a prompt.
                                    nonisolated private func chunkSourceCode(_ source: String) -> [String] {
                                        let maxChunkChars = 3000
                                        var chunks: [String] = []
                                        var currentFile = "Hal.swift"
                                        var currentBlock = "file header"
                                        var buffer: [String] = []
                                        ...
                                        func flush() {
                                            ...
                                            // Break the block body into <= maxChunkChars pieces on line
                                            // boundaries (never mid-line), so code stays readable.
                                            var pieces: [String] = []
                                            var pieceLines: [String] = []
                                            var pieceLen = 0
                                            for line in body.components(separatedBy: "\n") {
                                                if pieceLen + line.count + 1 > maxChunkChars && !pieceLines.isEmpty {
                                                    pieces.append(pieceLines.joined(separator: "\n"))
                                                    pieceLines.removeAll(keepingCapacity: true)
                                                    pieceLen = 0
                                                }
                                                pieceLines.append(line)
                                                pieceLen += line.count + 1
                                            }
                                            if !pieceLines.isEmpty { pieces.append(pieceLines.joined(separator: "\n")) }
                                            let total = pieces.count
                                            for (i, piece) in pieces.enumerated() {
                                                let partTag = total > 1 ? " | part \(i + 1)/\(total)" : ""
                                                let header = "[Hal source | FILE: \(currentFile) | \(currentBlock)\(partTag)]"
                                        ...
                                        for line in source.components(separatedBy: "\n") {
                                            if line.hasPrefix("// ==== FILE: ") {
                                                ...
                                                currentBlock = "file header"
                                                continue
                                            }
                                            if line.hasPrefix("// ==== LEGO START: ") {
                                                flush()
                                                currentBlock = "LEGO " + line
                                                    .replacingOccurrences(of: "// ==== LEGO START: ", with: "")
                                                    .replacingOccurrences(of: " ====", with: "")
                                                    .trimmingCharacters(in: .whitespaces)
                                                continue
                                            }
                                    ...
                                    // Background pass that fills the ACTIVE embedder's vector column for
                                    // any source_code chunk still missing it — after a fresh ingest, a
                                    // backend switch, or an interrupted prior pass. Idempotent and
                                    // resumable: it self-guards with a COUNT, so an ordinary launch (all
                                    // chunks already embedded) costs one query and returns. Runs off the
                                    // main thread via Task.detached; BM25 retrieval already works without
                                    // it, so semantic search simply sharpens once this completes.
                                    nonisolated private func ensureSourceCodeEmbeddings() {
                                        guard ensureHealthyConnection() else { return }
                                        ...
                                        var pending = 0
                                        var countStmt: OpaquePointer?
                                        if sqlite3_prepare_v2(db, "SELECT COUNT(*) FROM unified_content WHERE source_type = 'source_code' AND \(col) IS NULL;", -1, &countStmt, nil) == SQLITE_OK {
                                            if sqlite3_step(countStmt) == SQLITE_ROW { pending = Int(sqlite3_column_int(countStmt, 0)) }
                                        }
                                        sqlite3_finalize(countStmt)
                                        guard pending > 0 else { return }
                                        halLog("HALDEBUG-SELFKNOWLEDGE: embedding \(pending) source_code chunk(s) into \(col) in background")

                                        // Make sure the embedder is loaded before we embed (mirrors the
                                        // EmbedderMigrationCoordinator warm-up wait; capped so a bad load
                                        // can't hang the pass — chunks stay BM25-only until a later launch
                                        // retries).
                                        if !EmbeddingProvider.shared.isLoaded {
                                            EmbeddingProvider.shared.warmUp()
                                            let deadline = Date().addingTimeInterval(60)
                                            while !EmbeddingProvider.shared.isLoaded {
                                                if Date() > deadline {
                                                    halLog("HALDEBUG-SELFKNOWLEDGE: embedder not ready in 60s — source_code stays BM25-only for now")
                                                    return
                                                }
                                        ...
                                        if sqlite3_prepare_v2(db, "SELECT id, content FROM unified_content WHERE source_type = 'source_code' AND \(col) IS NULL;", -1, &sel, nil) == SQLITE_OK {
                                            while sqlite3_step(sel) == SQLITE_ROW {
                                                let id = String(cString: sqlite3_column_text(sel, 0))
                                                let content = String(cString: sqlite3_column_text(sel, 1))
                                        ...
                                        guard sqlite3_prepare_v2(db, updateSQL, -1, &upd, nil) == SQLITE_OK else { return }
                                        var filled = 0
                                        for row in rows {
                                            let vector = EmbeddingProvider.shared.embed(row.content, as: .document, in: backend) ?? []
                                            guard !vector.isEmpty else { continue }
                                            let bytes = vector.withUnsafeBufferPointer { Data(buffer: $0) }
                                            sqlite3_reset(upd)
                                            sqlite3_clear_bindings(upd)
                                            _ = bytes.withUnsafeBytes { rawBuf in
                                                sqlite3_bind_blob(upd, 1, rawBuf.baseAddress, Int32(bytes.count), nil)
                                            }
                                            let idNS = row.id as NSString
                                            sqlite3_bind_text(upd, 2, idNS.utf8String, -1, nil)
                                            if sqlite3_step(upd) == SQLITE_DONE { filled += 1 }
                                        }
                                    ...
                                    // own small `lab_reference` RAG document. Distinct from `source_code`: that gives
                                    // Hal an implementer's view (and, asked a Lab question, he confabulated Swift, seen
                                    // 2026-07-29); this gives him a user's guide. Built at RUNTIME from the live command
                                    // catalog (CommandCatalog.helpText) plus a hand-written three-doors + syntax + safety
                                    // primer, so it never drifts from the real verbs and needs no bundled file. Because
                                    ...
                                    nonisolated private func enableLabReferenceAccess() {
                                        // ONE SOURCE: the bundled HAL_GUIDE.md (the public user + developer guide) IS
                                        // Hal's self-knowledge of his own tools and architecture — the exact file the
                                        // in-app reader shows and the open-source repo ships, so there is no second
                                        // hand-written copy to drift. We split it on its top-level (## ) headings and
                                        // ingest each section under its OWN source_type, so an explicit Help topic
                                        // funnels ONLY its door (RoboRunner Help never surfaces the API/CLI section).
                                        // The COMMAND CATALOG stays a separate SHARED pile built from the live
                                        // CommandCatalog (the verb list is identical across all three doors and the
                                        // runtime catalog is always exact), so every tool topic can also draw on it.
                                        // `general`, `screens`, and `architecture` are auto-path only (see
                                        // HelpTopic.sourceTypes for the explicit-topic scopes).
                                        guard let guidePath = Bundle.main.path(forResource: "HAL_GUIDE", ofType: "md"),
                                              let guideRaw = try? String(contentsOfFile: guidePath, encoding: .utf8) else {
                                            print("HALDEBUG-SELFKNOWLEDGE: ERROR: HAL_GUIDE.md not bundled — cannot ingest Lab reference (hash left unset, will retry next launch)")
                                            return
                                        }

                                        // Split the guide into its top-level (## ) sections. Lines before the first
                                        // heading (the title + intro) collect under "__preamble__". A "### " subsection
                                        // heading is NOT a section break (hasPrefix("## ") is false for "### ").
                                        var guideSection: [String: String] = [:]
                                        var currentKey = "__preamble__"
                                        for line in guideRaw.components(separatedBy: "\n") {
                                            if line.hasPrefix("## ") {
                                                currentKey = String(line.dropFirst(3)).trimmingCharacters(in: .whitespaces)
                                            }
                                            guideSection[currentKey, default: ""] += line + "\n"
                                        }
                                        func part(_ heading: String) -> String {
                                            (guideSection[heading] ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
                                        }

                                        // The "general" pile is the orientation matter: the title/intro plus the two
                                        // framing sections (how the guide is organized, and the three-doors + Safe/
                                        // Advanced idea).
                                        let generalText = [
                                            part("__preamble__"),
                                            part("How this guide is organized"),
                                            part("The one idea behind the three doors")
                                        ].joined(separator: "\n\n")
                                        ...
                                        // One pile per section. Six are the guide's own sections (ingested verbatim,
                                        // so Hal reads exactly what a person reads); the catalog is the live shared pile.
                                        let sections: [(type: String, name: String, label: String, text: String)] = [
                                            ("lab_reference_general",      "Hal Guide - Overview",           "Hal Guide overview",           generalText),
                                            ("lab_reference_screens",      "Hal Guide - Screens & Settings", "Hal screens & settings guide", part("Screens and settings")),
                                            ("lab_reference_roborunner",   "Hal Guide - RoboRunner",         "Hal RoboRunner guide",         part("RoboRunner")),
                                            ("lab_reference_api",          "Hal Guide - Command API",        "Hal Developer API guide",      part("Command API")),
                                            ("lab_reference_cli",          "Hal Guide - Command Line",       "Hal CLI guide",                part("Command line")),
                                            ("lab_reference_architecture", "Hal Guide - Architecture",       "Hal architecture guide",       part("Architecture")),
                                            ("lab_reference_catalog",      "The Lab - Command Catalog",      "Hal command catalog",          catalogSection),
                                        ]

                                        // Fail loud if a guide-derived section came back empty — a renamed heading in
                                        // HAL_GUIDE.md would otherwise silently drop a whole door. Catalog is exempt.
                                        for s in sections where s.type != "lab_reference_catalog" {
                                            if s.text.isEmpty {
                                                print("HALDEBUG-SELFKNOWLEDGE: ERROR: guide section for \(s.type) is empty (heading renamed in HAL_GUIDE.md?) — hash left unset, will retry")
                                                return
                                            }
                                        }

                                        // Hash over every section's type+text, versioned so a change to the SPLIT
                                        // scheme (not just the text) forces a re-ingest. v3 = guide-sourced (six guide
                                        // sections + the live catalog), replacing the old inline four-piles blurbs.
                                        let combined = sections.map { $0.type + "\u{1}" + $0.text }.joined(separator: "\u{2}")
                                        let currentHash = (combined + "#labchunk-v3-guide").hash
                                        ...
                                        if currentHash == storedHash && storedHash != 0 && rowCount > 0 {
                                            print("HALDEBUG-SELFKNOWLEDGE: Lab reference unchanged, current")
                                            return
                                        }
                                        print("HALDEBUG-SELFKNOWLEDGE: (re)ingesting Lab usage reference from HAL_GUIDE.md (guide sections + live catalog)...")
...
extension MemoryStore {
    ...
    func searchUnifiedContent(
        ...
        FROM unified_content
        WHERE \(activeVectorColumn) IS NOT NULL\(semanticSourceClause)\(exclusionClause);
        """
        ...
        FROM unified_content_fts
        JOIN unified_content u ON u.rowid = unified_content_fts.rowid
        WHERE unified_content_fts MATCH ?\(bm25SourceClause)\(bm25ExclusionClause)
        ...
        AsyncThrowingStream { continuation in
            let genTask = Task {
                ...
                do {
                    ...
                    // Native <think> mode was removed 2026-07-22 in favor of the
                    // model-agnostic two-phase reasoning orchestrated in the ViewModel
                    // (see HalReasoning). We now ALWAYS ask the chat template for its
                    // non-thinking branch: enable_thinking=false so a Qwen-style
                    // template emits plain text (no <think>…</think>) for BOTH the
                    // reason pass and the answer pass. The visible "thinking" is phase
                    // 1's normal output, streamed into the panel by the ViewModel.
                    // Passing the key still matters: some templates (Qwen3, possibly
                    // Gemma 4) take a no-visible-output branch when it's absent.
                    ...
                    // Effective per-model settings. One sampler per model for BOTH
                    // phases (Mark's 2026-07-21 decision: one temperature per model,
                    // no phase split; stop chasing per-model reasoning recipes). The
                    // model's reasoningSettings are intentionally NOT merged in here.
                    let settings = self.currentModelConfig?.defaultSettings ?? ModelSettings()
                    // The live tuning overrides (ReasoningTuning) exist only for the
                    // test harness and apply ONLY in DEBUG builds — computed directly
                    // per build config so there is no constant-condition ternary (and
                    // thus no dead-branch warning). In RELEASE every override is nil,
                    // so the per-model `settings` are the sole source: no override can
                    // touch a real user's chat, even with the Developer API on.
                    #if DEBUG
                    let penaltyOverride = ReasoningTuning.shared.repetitionPenalty.map { Float($0) }
                    let tempOverride = ReasoningTuning.shared.temperatureOverride
                    let topPOverride = ReasoningTuning.shared.topPOverride
                    let topKOverride = ReasoningTuning.shared.topKOverride
                    let presenceOverride = ReasoningTuning.shared.presencePenaltyOverride
                    #else
                    let penaltyOverride: Float? = nil
                    let tempOverride: Double? = nil
                    let topPOverride: Float? = nil
                    let topKOverride: Int? = nil
                    let presenceOverride: Float? = nil
                    #endif
                    let modelPenalty = penaltyOverride ?? settings.repetitionPenalty
                    let modelPenaltyCtx = settings.repetitionContextSize ?? 20
                    // One temperature per model for both phases; the model's setting
                    // wins over the caller's default when present.
                    let effectiveTemperature = tempOverride ?? settings.temperature ?? temperature
                    let modelTopP = topPOverride ?? settings.topP ?? 1.0
                    let modelTopK = topKOverride ?? settings.topK ?? 0
                    let modelMinP = settings.minP ?? 0.0
                    let modelPresencePenalty = presenceOverride ?? settings.presencePenalty
                    halLog("HALDEBUG-MLX-CHAT: Sampler for \(self.currentModelConfig?.displayName ?? "unknown"): temp=\(String(format: "%.2f", effectiveTemperature)) topP=\(modelTopP) topK=\(modelTopK) minP=\(modelMinP) repPen=\(modelPenalty.map { String($0) } ?? "nil") presPen=\(modelPresencePenalty.map { String($0) } ?? "nil") maxTok=\(maxTokens.map(String.init) ?? "default")")
                    ...
                    //
                    // Explicit per-call cap wins (the two-phase REASON pass passes a
                    // depth budget here); else the DEBUG harness override; else 4096
                    // as the runaway ceiling. The hammer was cancelled — this token
                    // cap plus the two-phase structure replaced it.
                    let maxOutputTokens = maxTokens ?? ReasoningTuning.shared.maxOutputTokensOverride ?? 4096
                    ...
                    #if DEBUG
                    ReasoningTuning.shared.resetRawStream()
                    #endif
                    ...
                    streamLoop: while let event = await iterator.next() {
                        ...
                        case .chunk(let text):
                            ...
                            #if DEBUG
                            ReasoningTuning.shared.appendRawStream(text)
                            #endif
                    ...
                    #if DEBUG
                    do {
                        // DEBUG only: log the RAW generation trace so it's queryable
                        // via GET_LOGS for the test harness. Two chunks so a loop is
                        // visible near its start.
...
enum HelpTopic: String, CaseIterable, Identifiable {
    ...
    /// Architecture draws on the guide's architecture prose plus Hal's own source. The catalog is intentionally shared — a
    ...
    var sourceTypes: [String] {
        ...
        case .architecture: return ["lab_reference_architecture", "source_code"]
        }
    }
}
...
class LLMService: ObservableObject {
    ...
    func generateChatResponseStream(messages: [HalChatMessage], temperature: Double = 0.7, maxTokens: Int? = nil) -> AsyncThrowingStream<String, Error> {
        ...
        case .mlx:
            ...
            if mlxWrapper.isModelLoaded {
                return mlxWrapper.generateChatStream(messages: messages, temperature: temperature, maxTokens: maxTokens)
            }
...
struct ModelLibraryView: View {
    // Always a drill-down push from Settings now (system back chevron handles dismiss). It once
    // doubled as an API-path modal that needed its own "Done", but that call site is gone; the
    // isModal flag + conditional Done were removed 2026-08-05 as dead code.
...
// ---- Model-agnostic two-phase reasoning (2026-07-22) --------------------------
// Hal's "show me your reasoning" mode works on EVERY model, not just the ones
// with native <think> tokens. When the brain toggle is on, the ViewModel runs a
// user turn as two passes: (1) a REASON pass with full context that streams into
// the collapsible thinking panel, bounded to a depth budget; (2) a CONCLUDE pass
// with only persona + the bounded reasoning (no RAG) that writes the answer. This
// replaced the old native-<think>-only path entirely. Prompts + budget are ported
...
enum HalReasoning {
    ...
    /// narrating or quoting it. Verbatim from the harness CONCLUDE_PROMPT; this
    /// framing is what fixed the phase-2 voice bug (Hal answers AS himself instead
    /// of commenting on "the reasoning").
    static func concludePrompt(question: String, reasoning: String) -> String {
        """
        A user asked you: "\(question)"

        You have already thought it through. These are your PRIVATE thoughts — the user cannot see them, and you must not quote, mention, or describe them:
        \"\"\"
        \(reasoning)
        \"\"\"

        Now reply to the user directly and concisely, in your own voice, as Hal — the answer itself, as if speaking to them. Do not talk about your reasoning, your notes, or your process.
        """
    }

    /// Bound the captured reasoning to the depth budget before it goes into the
    /// conclude prompt, cutting on a sentence/line boundary when possible (ports
    /// the harness `truncate_clean`). Keeps a runaway reason pass from blowing the
    /// phase-2 prompt budget.
    static func bound(_ text: String, budget tokenBudget: Int) -> String {
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        let charBudget = tokenBudget * charsPerToken
        guard trimmed.count > charBudget else { return trimmed }
        let cut = String(trimmed.prefix(charBudget))
        for sep in ["\n", ". ", "! ", "? ", "; "] {
            if let r = cut.range(of: sep, options: .backwards),
               cut.distance(from: cut.startIndex, to: r.lowerBound) > charBudget / 2 {
                return String(cut[..<r.upperBound]).trimmingCharacters(in: .whitespacesAndNewlines)
            }
        }
        return cut.trimmingCharacters(in: .whitespacesAndNewlines)
    }

    /// Defensive: strip any stray <think>/</think> tags a natively-reasoning model
    /// might still emit in the reason pass even with enable_thinking=false, so the
    /// thinking panel shows clean prose.
...
class ChatViewModel: ObservableObject {
    ...
    // Nested one deeper still: opens RoboRunner's Check (Script Check) sheet from inside the editor,
    // so the antenna can screenshot/verify the coach's problem list without a human tapping Check.
    @Published var apiNavRoboIssues: Bool = false
    ...
    var effectiveSystemPrompt: String {
        ...
        // NOTE (2026-07-22): the old Layer-0 "<think> … </think>" directive was
        // removed here. Reasoning is now the model-agnostic two-phase flow (see
        // HalReasoning + the send path): the reason instruction rides on the phase-1
        // USER turn, and phase 2 gets this clean persona plus the bounded reasoning.
        // Injecting a <think> directive into the system prompt would poison both.
        return base
    }
    ...
    //
    // User-facing, sticky (persists across launches). Works on EVERY model now
    // (the old isReasoningModel gate is gone, 2026-07-22). When on, a user turn
    // runs as the two-phase HalReasoning flow: a bounded REASON pass streams into
    // the collapsible "Thinking" panel, then a CONCLUDE pass writes the answer
    // from that reasoning. Surfaced as a brain toggle beside the lock. See
    ...
    func classifySelfReferential(_ question: String) async -> Bool {
        await isSelfReferentialQuestion(userInput: question)
                                                                        ...
                                                                        func buildChatMessages(currentInput: String,
                                                                            ...
                                                                            // or truncated).
                                                                            @MainActor
                                                                            // Fast, non-lossy budget cap for the KnowledgeRouter's verbatim
                                                                            // injections (source code, Lab guide). Unlike resolveSegment, this
                                                                            // never calls the LLM summarizer — reference material and code must
                                                                            // stay verbatim, and the ~20s AFM summarize latency is unacceptable.
                                                                            // Keeps the head (the highest-relevance chunks / the guide's primer)
                                                                            // and marks the cut honestly.
                                                                            func truncateToBudget(_ text: String, maxChars: Int) -> String {
...
// ==== LEGO START: 24 ChatViewModel (Send Message Flow) ====
                                                                ...
                                                                private func runSingleModelTurn(userInput: String, historyMessagesOverride: [ChatMessage]? = nil, skipUserMessage: Bool = false) async {
                                                                    ...
                                                                    do {
                                                                        ...
                                                                        // Two-phase reasoning (model-agnostic). When the brain is on,
                                                                        // phase 1 builds FULL context around a REASON instruction and
                                                                        // streams into the thinking panel; phase 2 answers from the
                                                                        ...
                                                                        finalText = ""
                                                                        // Captured phase-1 reasoning for the thinking panel; nil when the
                                                                        // brain is off. Set inside the two-phase branch below.
                                                                        var capturedThinking: String? = nil
                                                                        let turnTemp = temperature  // one temperature per model (from the model's settings); this is the caller default
                                                                        do {
                                                                            if isReasoning {
                                                                                // ===== PHASE 1 — REASON (full context, bounded to depth) =====
                                                                                // Streams live into the collapsible thinking panel. The depth
                                                                                ...
                                                                                // ===== PHASE 2 — CONCLUDE (persona + reasoning only, no RAG) =====
                                                                                // Minimal messages: clean persona + the private reasoning. The
                                                                                // answer streams into the bubble in Hal's own voice.
                                                                                let concludeMessages: [HalChatMessage] = [
                                                                        ...
                                                                        // The answer (phase 2, or the single pass) is what flows into the
                                                                        // message, memory, and RAG. The reasoning (phase 1, captured above)
                                                                        // goes only to the collapsible panel and is never stored.
                                                                        let text = cleanedFull
                                                                        let thinkingText: String? = capturedThinking
                                                                        ...
                                                                        await MainActor.run {
                                                                            ...
                                                                            // Help Mode (B-a): a help session is MEMORY-EXEMPT. The answer is
                                                                            // already shown on screen (settled in the MainActor.run above); return
                                                                            // here so this turn skips ALL persistence and derived self-knowledge —
                                                                            // storeTurn/RAG, the artifacts, reflection, consolidation, and the
                                                                            // auto-summary — leaving no trace in memory. It stays visible for the
                                                                            // session only (like the entry/exit narration, it is not stored).
                                                                            if let ht = self.helpTopic {
                                                                                halLog("HALDEBUG-HELP: help-mode turn (\(ht.rawValue)) — not persisted to memory")
                                                                                return
                                                                            }
                                                                ...
                                                                private func runSalonTurn(userInput: String) async {
                                                                    ...
                                                                    // Help Mode (B-a): a help session is memory-exempt — skip the salon
                                                                    // user-message store so a help exchange leaves no trace in memory.
                                                                    if helpTopic == nil {
                                                                ...
                                                                private func runSalonSeat(userInput: String, seatPosition: Int, baselineHistory: [ChatMessage]) async {
                                                                    ...
                                                                    do {
                                                                        ...
                                                                        // sequential thinking.
                                                                        finalText = ""
                                                                        // Salon still uses the defensive <think> split (harmless: with
                                                                        // native thinking removed, plain output has no tags to split).
                                                                        // Two-phase reasoning for salon seats is deferred (2026-07-22) —
                                                                        // tracked in NEXT; salon runs the normal single pass for now.
                                                                        let isReasoning = self.reasoningActive
                                                                        let turnTemp = temperature  // one temperature per model (from the model's settings); this is the caller default
                                                                        do {
                                                                            let stream = llmService.generateChatResponseStream(messages: chatMessages, temperature: turnTemp)
                                                                        ...
                                                                        await MainActor.run {
                                                                            ...
                                                                            // Help Mode (B-a): a help session is memory-exempt — skip the salon
                                                                            // seat store so a help exchange leaves no trace in memory.
                                                                            if self.helpTopic == nil {
                                                                                self.memoryStore.storeTurn(
                                                                                    conversationId: self.conversationId,
                                                                                    userMessage: userInput,
                                                                                    assistantMessage: text,
