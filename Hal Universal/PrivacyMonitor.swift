// PrivacyMonitor.swift
// Hal Universal
//
// The privacy lock indicator's engine and UI. Surfaces - in the chat
// toolbar - that Hal's inference runs entirely on-device.
//
// EVERY model Hal runs is on-device: local MLX models, and Apple Intelligence
// via `SystemLanguageModel.default`, which runs strictly on-device and never
// routes to Apple's Private Cloud Compute. PCC is reachable only by explicitly
// instantiating `PrivateCloudComputeLanguageModel` (a WWDC26 addition), which
// Hal does NOT use. (Corrected 2026-07-18 after verifying against Apple's WWDC26
// Foundation Models sessions — the earlier "AFM + network → cloud possible"
// assumption was wrong; `SystemLanguageModel.default` does not self-route to PCC.)
//
// So the indicator reads on-device. The network monitor + truth table are kept
// so that IF a cloud-capable model source is ever added (e.g. an opt-in PCC
// model), the indicator unlocks correctly for it.
//
// Three pieces live here:
//   1. `PrivacyMonitor` - an ObservableObject wrapping `NWPathMonitor`. Its
//      only published state is `isNetworkAvailable`. It defaults to `false`
//      (locked-is-the-safe-default) until the first network path arrives,
//      then flips within ~1s of any change.
//   2. `PrivacyMonitor.isLocked(...)` - a PURE, testable function holding the
//      lock/unlock truth table. Kept free of catalog/UI dependencies so the
//      logic can be reasoned about (and unit-tested) in isolation; the caller
//      resolves salon seat sources and passes them in.
//   3. `PrivacyLockPopover` - the small explain-yourself popover shown on tap.
//
// The glyph itself (lock / lock.open) and the ToolbarItem live in
// ChatViews.swift's iOSChatView, computed live from the active model + this
// monitor's network state + the salon config, so it updates the instant the
// user switches models or toggles Airplane Mode.

import SwiftUI
import Network
import Combine

// ==== LEGO START: 41 Privacy Lock (Network Monitor, Lock Truth Table, Popover) ====

/// Watches the device's network reachability for the privacy lock indicator.
/// A single shared instance is started once at app launch.
final class PrivacyMonitor: ObservableObject {
    static let shared = PrivacyMonitor()

    /// True when a usable network path exists (Wi-Fi, cellular, wired, or VPN).
    /// Retained for the truth table's future cloud-source case; today it does
    /// not change the lock outcome, since every model Hal runs is on-device.
    /// Starts `false` (the safe default) until the first `NWPathMonitor` update.
    @Published private(set) var isNetworkAvailable: Bool = false

    private let monitor = NWPathMonitor()
    private let queue = DispatchQueue(label: "com.MarkFriedlander.Hal.PrivacyMonitor")
    private var started = false

    private init() {}

    /// Begin monitoring. Idempotent — safe to call more than once. Call once
    /// at app launch (see Hal10000App.init).
    func start() {
        guard !started else { return }
        started = true
        monitor.pathUpdateHandler = { [weak self] path in
            // `.satisfied` means a usable path exists. VPN reports satisfied
            // (it IS a usable, cloud-capable path — correctly treated as
            // network-available). `.unsatisfied` / `.requiresConnection`
            // (Airplane Mode, Wi-Fi up but unreachable, cellular off) → not
            // available → the lock closes.
            let available = (path.status == .satisfied)
            DispatchQueue.main.async {
                guard let self else { return }
                if self.isNetworkAvailable != available {
                    self.isNetworkAvailable = available
                }
            }
        }
        monitor.start(queue: queue)
    }

    // MARK: - Lock decision (pure truth table)

    /// The lock/unlock decision, as a pure function of the inputs. See the
    /// truth table above. `salonSeatSources` is the resolved `ModelSource` of
    /// each ACTIVE salon seat (the caller looks these up; an unresolvable seat
    /// is passed as `.appleFoundation`, which is on-device — harmless today,
    /// since every source Hal has is on-device).
    ///
    /// Returns `true` = locked (no data can leave), `false` = unlocked
    /// (a cloud-capable source is active — not possible today, kept for the
    /// future-cloud-source case).
    static func isLocked(
        activeModelSource: ModelSource,
        networkAvailable: Bool,
        salonEnabled: Bool,
        salonSeatSources: [ModelSource]
    ) -> Bool {
        // No network → nothing can leave the device, whatever the model is.
        // (Belt-and-suspenders: today every source below is on-device anyway.)
        guard networkAvailable else { return true }

        // Salon: locked only if every active seat runs on-device.
        if salonEnabled && !salonSeatSources.isEmpty {
            return salonSeatSources.allSatisfy(Self.isOnDevice)
        }

        // Single-model path.
        return Self.isOnDevice(activeModelSource)
    }

    /// Whether a model source runs entirely on-device. BOTH of Hal's sources do:
    /// local MLX models, and Apple Intelligence via `SystemLanguageModel.default`,
    /// which runs strictly on-device and never routes to Private Cloud Compute
    /// (Hal does not use the opt-in `PrivateCloudComputeLanguageModel`). If a
    /// cloud-capable source is ever added, return `false` for it here.
    nonisolated private static func isOnDevice(_ source: ModelSource) -> Bool {
        switch source {
        case .mlx, .appleFoundation:
            return true
        }
    }

}


// MARK: - PrivacyLockPopover (tap explanation)

/// The small popover shown when the user taps the lock. Explains the current
/// state in plain language and offers a one-tap jump to the Model Library.
struct PrivacyLockPopover: View {
    let isLocked: Bool
    let modelName: String
    /// Invoked when the user taps "Model Library" — the caller dismisses the
    /// popover and presents the library.
    let onOpenModelLibrary: () -> Void

    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 8) {
                Image(systemName: isLocked ? "lock" : "lock.open")
                Text(isLocked ? "On-device" : "Cloud possible")
                    .fontWeight(.semibold)
            }
            .font(.headline)

            Text(explanation)
                .font(.subheadline)
                .foregroundColor(.secondary)
                .fixedSize(horizontal: false, vertical: true)

            Button(action: onOpenModelLibrary) {
                HStack(spacing: 4) {
                    Text("Model Library")
                    Image(systemName: "arrow.right")
                }
                .font(.subheadline.weight(.medium))
            }
            .padding(.top, 2)
        }
        .padding(16)
        .frame(width: 280)
    }

    private var explanation: String {
        if isLocked {
            return "Hal is fully on-device right now. The active model (\(modelName)) runs on this device — no request you send and no response you receive leaves it. Apple Intelligence uses Apple's on-device model and does not route to Private Cloud Compute."
        } else {
            // Currently unreachable — Hal runs only on-device models. Kept in
            // case a cloud-capable model source is ever added.
            return "A cloud-capable model is active, so some requests may leave this device. To stay fully on-device, pick a downloaded local model from the Model Library or use Apple Intelligence."
        }
    }
}

// ==== LEGO END: 41 Privacy Lock (Network Monitor, Lock Truth Table, Popover) ====
