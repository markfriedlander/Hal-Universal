...
struct ChatBubbleView: View {
    ...
    // Read-aloud state, so the speaker icon in the message action row can show active
    // (filled) while THIS message is being spoken, and toggle stop.
    ...
    // MARK: - Message action row (visible affordances mirroring the context menu)
    //
    // A row of small tappable icons under the footer, one per context-menu action, so the
    // actions are DISCOVERABLE (and mouse-clickable on Mac) instead of hidden behind a
    // long-press / right-click. Shown on ALL devices (Mark 2026-08-02 — cross-platform makes
    // them more discoverable everywhere); the context menu stays too. Hal's turns get the full
    // set (four copies + inline details + prompt details + read-aloud). Not shown on partial or
    // status rows. NOTE: this mirrors the assistant contextMenu (~1546) by hand for now — a
    // later refactor could drive both from one action list (single source); kept simple for v1
    // since Mark plans to whittle the set after living with it.
    @ViewBuilder private var messageActionRow: some View {
        HStack(spacing: 20) {
            actionIcon("doc.on.doc", "Copy message") {
                UIPasteboard.general.string = message.content
            }
            actionIcon("doc.on.doc.fill", "Copy thread") {
                UIPasteboard.general.string = chatViewModel.exportChatHistory()
            }
            actionIcon("doc.text.magnifyingglass", "Copy message with details") {
                UIPasteboard.general.string = buildDetailsShareText()
            }
            actionIcon("doc.text.fill", "Copy thread with details") {
                UIPasteboard.general.string = chatViewModel.exportChatHistoryDetailed()
            }
            if !message.isFromUser {
                actionIcon(chatViewModel.showInlineDetails ? "info.circle.fill" : "info.circle",
                           "Toggle inline details") {
                    chatViewModel.showInlineDetails.toggle()
                }
                actionIcon("text.magnifyingglass", "Prompt details") {
                    showingPromptDetail = true
                }
                let speaking = speech.isSpeaking && speech.speakingMessageID == message.id.uuidString
                actionIcon(speaking ? "speaker.wave.2.fill" : "speaker.wave.2",
                           speaking ? "Stop reading" : "Read aloud") {
                    speech.toggle(message.content, messageID: message.id.uuidString)
                }
            }
        }
        .font(.caption)
    ...
    private func actionIcon(_ systemName: String, _ label: String,
                            _ action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Image(systemName: systemName).contentShape(Rectangle())
        }
        .buttonStyle(.plain)
            ...
            if message.isFromUser {
                ...
                VStack(alignment: .trailing, spacing: 0) {
                    ...
                    footerView
                        .padding(.horizontal, chatDensity.assistantHInset)
                        .frame(maxWidth: assistantMaxWidth, alignment: .leading)
                    // Visible action row on its own line under the footer text (all devices).
                    if !message.isPartial && !isStatusMessage {
                        messageActionRow
                            .padding(.horizontal, chatDensity.assistantHInset)
                            .frame(maxWidth: assistantMaxWidth, alignment: .leading)
                    }
...
struct MarkdownView: View {
    ...
    private func blockView(_ block: MDBlock) -> some View {
        ...
        case .codeBlock(let code):
            // Render fenced code as a distinct block, not "just small type":
            //  - a readable monospaced size that scales with the chosen body size
            //    (was .caption, which read as tiny),
            //  - a systemGray5 fill that stays visibly distinct in BOTH light and
            //    dark (systemGray6 was ~invisible against the dark chat background),
            //  - a hairline border so the box reads as a box regardless of contrast,
            //  - horizontal scroll so long lines keep their formatting instead of
            //    wrapping mid-token,
    ...
    private func inlineText(_ s: String) -> Text {
        if var attr = try? AttributedString(
            ...
            // Give inline code spans (`like this`) a distinct look instead of
            // leaving them as ordinary prose: a monospaced font sized to sit with
            // the surrounding text, and a faint theme-aware highlight behind the
            // glyphs so `code` reads as code mid-sentence. AttributedString parses
            // backticks into an `.code` inline intent; we restyle those runs.
