...
**Updated:** July 11, 2026 (latest — **Apple Watch excision Stage 1**: after re-researching and confirming the watch→iPhone relay is impossible on iOS [Metal is foreground-only; watchOS 27 went cloud/PCC], all iOS-side watch Swift was removed and device-build-verified [commit `1aae4f6`]. Stages 2 [Mark deletes the Xcode watch targets] + 3 [delete watch source dirs] remain — see NEXT "Apple Watch excision — staged". Earlier: WL1-7 all landed, M1 softened to mixed, one-source-of-truth fix for Posey shared models, item 6 Bonsai ships. See MEMORY current-state for the full list.)
**Branch:** `main`
