...
**Updated:** July 11, 2026 (latest — **Apple Watch companion fully excised**: after re-researching and confirming the watch→iPhone relay is impossible on iOS [Metal is foreground-only; watchOS 27 went cloud/PCC], all three stages landed same-day — iOS-side Swift removed [CC], watch targets + framework link deleted [Mark, Xcode], source folders + leftover embed phase gone. `grep -i watch` over the project is empty; device build clean. Also simplified `sendMessage(externalText:)` → `sendMessage()` [param only served the watch bridge]. Earlier: WL1-7 all landed, M1 softened to mixed, one-source-of-truth fix for Posey shared models, item 6 Bonsai ships. See MEMORY current-state.)
**Branch:** `main`
