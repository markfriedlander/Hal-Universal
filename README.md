...
**Apple Intelligence:** Apple Intelligence is Apple's own system, and only Apple decides when things are processed in the cloud, so while you're online, we can't guarantee a request stays on your device. To guarantee private, on-device-only behavior, choose a local model or shut off all networks.
